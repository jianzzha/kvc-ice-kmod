...
build: login_registry
	podman build --build-arg KVER=${KERNEL_VERSION} --build-arg DRIVER_TOOLKIT_IMAGE=${DRIVER_TOOLKIT_IMAGE} -t ${REGISTRY}/ice-kmod-driver-container:demo -f Dockerfile.ice .
	podman push ${REGISTRY}/ice-kmod-driver-container:demo

deploy: pull_secret registry_cert build
	export REGISTRY=$(REGISTRY) NODE_LABEL=$(NODE_LABEL) ;\
	envsubst < ice-install.yaml.template > ice-install.yaml
	oc create -f ice-install.yaml

destroy: ice-install.yaml
	oc delete -f ice-install.yaml
